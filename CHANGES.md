...
- **v132.6 — phone alignment pass.** Every page was walked at phone width and tidied: three-tile rows (My Day, Tickets, Team cards) sit in one line, odd leftover KPI tiles stretch across instead of dangling, KPI icons give way to the label so nothing wraps; the profile hero stacks avatar → name → a full-width action row; Alerts tabs scroll in one row instead of a ragged wrap; Access Control → People turns into stacked cards with a full-width role picker; the role editor puts each area's toggles under its name; filter rows put the search box on its own line; calendar cells keep the hours and drop the in/out times; page headers no longer leave a blank gap above their buttons.
