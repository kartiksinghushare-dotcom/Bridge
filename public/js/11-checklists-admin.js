...
let list=can('allChecklists','view')?DB.checklists:DB.checklists.filter(c=>c.createdBy===S.uid||(c.assignees||[]).includes(S.uid));
  ...
  <div class="space-y-2.5">
    ${list.map(c=>{
      ...
      return`<div class="bg-white rounded-2xl border border-ink-100 shadow-soft p-4"><div class="flex items-start gap-3"><div class="flex-1 min-w-0"><div class="flex items-center gap-2 flex-wrap mb-1"><span class="text-[11px] font-bold px-2 py-0.5 rounded-full ${active?'bg-brand-50 text-brand-700':'bg-ink-100 text-ink-400'}">${esc(c.frequency)}</span><span class="text-xs text-ink-400">${esc(c.department)}</span>${locs.length?`<span class="text-xs text-sky-600 flex items-center gap-1">${ic('pin','w-3 h-3')}${locs.map(l=>esc(l.name)).join(', ')}</span>`:''}</div><h3 class="fd font-bold">${esc(c.name)}${c.status==='Draft'?'<span style="font-size:10px;font-weight:800;padding:1px 7px;border-radius:20px;background:#F8F0DE;color:#6B4E1F;margin-left:6px;vertical-align:middle">DRAFT</span>':''}</h3><p class="text-xs text-ink-400 mt-0.5 line-clamp-1">${esc(c.description||'')}</p><div class="flex items-center gap-3 mt-1.5 text-xs text-ink-400"><span>${ic('clock','w-3 h-3 inline mr-0.5')}${esc(c.schedule||c.frequency)}${c.scheduleTime?' · due '+c.scheduleTime:''}</span>${c.startDate?`<span>${ic('doc','w-3 h-3 inline mr-0.5')}${fmtS(c.startDate)}${c.endDate?' → '+fmtS(c.endDate):''}</span>`:''}</div></div><div class="flex flex-col items-end gap-2 shrink-0">${(can('checklists','edit')||can('checklists','duplicate')||can('checklists','delete'))?`<div class="flex gap-1">${can('checklists','edit')?`<button onclick="App.editCl('${c.id}')" title="Edit" style="width:28px;height:28px;display:grid;place-items:center;border-radius:8px;color:#A59788;background:transparent;border:none;cursor:pointer" onmouseover="this.style.background='#F4F0EA'" onmouseout="this.style.background='transparent'">${ic('edit','w-3.5 h-3.5')}</button>`:''}${can('checklists','duplicate')?`<button onclick="App.dupCl('${c.id}')" title="Duplicate" style="width:28px;height:28px;display:grid;place-items:center;border-radius:8px;color:#A59788;background:transparent;border:none;cursor:pointer" onmouseover="this.style.background='#EEE5D6';this.style.color='#96695B'" onmouseout="this.style.background='transparent';this.style.color='#A59788'">${ic('copy','w-3.5 h-3.5')}</button>`:''}${can('checklists','delete')?`<button onclick="App.delCl('${c.id}')" title="Delete" style="width:28px;height:28px;display:grid;place-items:center;border-radius:8px;color:#A59788;background:transparent;border:none;cursor:pointer" onmouseover="this.style.background='#F9EBE5';this.style.color='#A63528'" onmouseout="this.style.background='transparent';this.style.color='#A59788'">${ic('trash','w-3.5 h-3.5')}</button>`:''}</div>`:''}<div class="flex -space-x-1">${ass.slice(0,4).map(u=>`<div class="ring-2 ring-white rounded-full">${avatar(u,'w-6 h-6','text-[9px]')}</div>`).join('')}</div></div></div></div>`;
    }).join('')}
...
function _renderClModal(editing){
  ...
  openModal(`<div style="display:flex;flex-direction:column;max-height:92vh">
    ...
    <div class="flex items-center justify-between px-5 pt-5 pb-4 border-b border-ink-100">
      ...
      <button onclick="App.closeModal()" style="width:32px;height:32px;display:grid;place-items:center;border-radius:10px;background:transparent;border:none;cursor:pointer;color:#A59788" onmouseover="this.style.background='#F7F3EE'" onmouseout="this.style.background='transparent'">${ic('x')}</button>
    </div>
    <!-- Scrollable body -->
    <div style="overflow-y:auto;flex:1;padding:16px 20px;display:flex;flex-direction:column;gap:20px">
      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          <label class="block text-xs font-bold text-ink-500 uppercase tracking-wide">Questions <span style="color:#C25441">*</span></label>
          <button type="button" onclick="App._openClQuestionPicker()" style="display:inline-flex;align-items:center;gap:5px;font-size:12px;font-weight:700;padding:5px 12px;border-radius:8px;background:#54433C;color:#fff;border:none;cursor:pointer">${ic('plus','w-3 h-3')}Add question</button>
        </div>
        <div id="cn-questions" style="display:flex;flex-direction:column;gap:6px">
          ${(c.questionIds||[]).map(qid=>{const q=(DB.questions||[]).find(x=>x.id===qid);if(!q)return'';const tl=(Q_TYPES.find(t=>t.id===q.type)||{label:q.type}).label;const clr=Q_TYPE_CLR[q.type]||'#786A5F';const bg=Q_TYPE_BG[q.type]||'#F7F3EE';const cfg=(c.questionConfigs||{})[qid]||{};const escCount=Object.values(cfg).filter(v=>v).length;return`<div style="display:flex;align-items:center;gap:8px;background:#FAF7F3;border:1px solid #E6DED3;border-radius:10px;padding:8px 12px"><span style="font-size:10px;font-weight:700;padding:2px 7px;border-radius:6px;background:${bg};color:${clr};flex-shrink:0">${tl}</span><span style="flex:1;font-size:13px;font-weight:500;color:#13171B;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(q.text)}</span>${escCount?`<span style="font-size:10px;font-weight:700;color:#C25441;flex-shrink:0">${escCount} escalation${escCount>1?'s':''}</span>`:''}<button type="button" onclick="App._editClQuestionEscalation('${qid}')" style="font-size:11px;font-weight:600;padding:3px 8px;border-radius:6px;border:1px solid #E6DED3;background:#fff;cursor:pointer;color:#3A312A;flex-shrink:0">Escalation</button><button type="button" onclick="App._removeClQuestion('${qid}')" style="width:20px;height:20px;display:grid;place-items:center;border-radius:5px;border:none;background:transparent;color:#D5C9BC;cursor:pointer;flex-shrink:0" onmouseover="this.style.color='#A63528'" onmouseout="this.style.color='#D5C9BC'">${ic('x','w-3 h-3')}</button></div>`;}).join('')}
          ${!(c.questionIds||[]).length?`<div style="text-align:center;padding:12px;border:2px dashed #E6DED3;border-radius:10px;font-size:12px;color:#A59788">No questions added yet</div>`:''}
        </div>
      </div>
      <!-- Assign -->
      <div>
        ...
        <div class="bg-ink-50 rounded-xl p-3 mb-2">
          ...
          <p style="font-size:11px;color:#A59788;margin-top:2px">On: one submission by any assignee completes it for everyone. Off: every assignee must submit individually.</p>
        </div>
...
function _freqUI(freq){
  ...
  if(freq==='Daily'){
    return`<div>
      <div style="font-size:10px;font-weight:700;color:#A59788;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Schedule</div>
      <div style="display:flex;gap:6px;margin-bottom:10px">
        ${['Every day','Selected weekdays'].map(o=>`<button type="button" data-sched="${o}" onclick="App._dailySched('${o}',this)"
          style="flex:1;padding:8px;border-radius:10px;border:1.5px solid;font-size:13px;font-weight:600;cursor:pointer;transition:all .12s;background:${sched===o?'#13171B':'#fff'};color:${sched===o?'#fff':'#786A5F'};border-color:${sched===o?'#13171B':'#EDE7DC'}">${o}</button>`).join('')}
      </div>
  ...
  if(freq==='Weekly'){
    return`<div>
      <div style="font-size:10px;font-weight:700;color:#A59788;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Repeat on</div>
  ...
  if(freq==='Monthly'){
    ...
    return`<div>
      <div style="font-size:10px;font-weight:700;color:#A59788;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Days of month</div>
      <div style="display:flex;flex-wrap:wrap;gap:5px;margin-bottom:6px">
        ${nums.map(n=>{const on=sdtNorm.includes(n);return`<button type="button" onclick="App._togDN('${n}',this)"
          style="min-width:${n==='L'?48:32}px;height:32px;border-radius:50%;border:1.5px solid;font-size:12px;font-weight:600;cursor:pointer;transition:all .12s;background:${on?'#13171B':'#fff'};color:${on?'#fff':'#786A5F'};border-color:${on?'#13171B':'#EDE7DC'};${n==='L'?'border-radius:8px;padding:0 8px':''}" title="${n==='L'?'Last day of month':n}">${n==='L'?'Last':n}</button>`;}).join('')}
      </div>
      <p style="font-size:11px;color:#A59788">Tip: Select "Last" to always run on the last day of any month (handles 28/29/30/31 automatically)</p>
    </div>`;
  }
  ...
  if(freq==='Custom'){
    ...
    return`<div>
      <div style="font-size:10px;font-weight:700;color:#A59788;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Pick dates</div>
      <div style="background:#F7F3EE;border-radius:10px;padding:8px;display:inline-block;min-width:220px">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
          <button type="button" onclick="if(!CLD._calYear)CLD._calYear=${cy};if(!CLD._calMonth&&CLD._calMonth!==0)CLD._calMonth=${cm};CLD._calMonth--;if(CLD._calMonth<0){CLD._calMonth=11;CLD._calYear--;}$('#cn-sched').innerHTML=_freqUI('Custom')" style="width:22px;height:22px;border-radius:6px;border:1px solid #EDE7DC;background:#fff;cursor:pointer;display:grid;place-items:center;color:#786A5F">${ic('back','w-3 h-3')}</button>
          <span style="font-size:12px;font-weight:700;color:#13171B">${MN[calMonth]} ${calYear}</span>
          <button type="button" onclick="if(!CLD._calYear)CLD._calYear=${cy};if(!CLD._calMonth&&CLD._calMonth!==0)CLD._calMonth=${cm};CLD._calMonth++;if(CLD._calMonth>11){CLD._calMonth=0;CLD._calYear++;}$('#cn-sched').innerHTML=_freqUI('Custom')" style="width:22px;height:22px;border-radius:6px;border:1px solid #EDE7DC;background:#fff;cursor:pointer;display:grid;place-items:center;color:#786A5F">${ic('chevR','w-3 h-3')}</button>
        </div>
        <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:1px;margin-bottom:2px;text-align:center">
          ${['S','M','T','W','T','F','S'].map(d=>`<div style="font-size:9px;font-weight:700;color:#A59788;padding:2px 0">${d}</div>`).join('')}
        </div>
        ${weeks.map(w=>`<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:1px">${w.map(d=>{if(!d)return'<div></div>';const iso=calYear+'-'+String(calMonth+1).padStart(2,'0')+'-'+String(d).padStart(2,'0');const on=cdt.includes(iso);const past=iso<todayISO();return`<button type="button" onclick="App._togCalDate('${iso}')" style="aspect-ratio:1;border-radius:6px;border:none;font-size:11px;font-weight:${on?700:400};cursor:pointer;background:${on?'#13171B':'transparent'};color:${on?'#fff':past?'#D5C9BC':'#13171B'};padding:3px 0">${d}</button>`;}).join('')}</div>`).join('')}
      </div>
...
App._dailySched=(o,el)=>{
  ...
  el.parentNode.querySelectorAll('button[data-sched]').forEach(b=>{
    ...
    b.style.borderColor=active?'#13171B':'#EDE7DC';
  });
...
App._togDN=(n,el)=>{
  ...
  el.style.background=on?'#13171B':'#fff';el.style.color=on?'#fff':'#786A5F';el.style.borderColor=on?'#13171B':'#EDE7DC';
};
App._togCalDate=(iso)=>{
