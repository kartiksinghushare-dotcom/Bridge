...
function myClsPage(){
  ...
  <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:14px">
    <div>
      <h1 class="fd" style="font-size:24px;font-weight:800;color:#13171B;letter-spacing:-.5px">My Checklists</h1>
      <p style="font-size:13px;color:#A8998A;margin-top:2px">${new Date(sel+'T00:00:00').toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long'})}</p>
    </div>
    <div style="display:flex;gap:3px;align-items:center;margin-top:2px">
      <button onclick="(()=>{S.calWk--;const ref2=new Date(todayISO()+'T00:00:00');const dow2=ref2.getDay();ref2.setDate(ref2.getDate()+(dow2===0?-6:1-dow2)+S.calWk*7);const oldDow=new Date(S.calDate+'T00:00:00').getDay()||7;const nd=new Date(ref2);nd.setDate(nd.getDate()+oldDow-1);S.calDate=nd.getFullYear()+'-'+String(nd.getMonth()+1).padStart(2,'0')+'-'+String(nd.getDate()).padStart(2,'0');S.expandedCl=null;rr();App._lazyLoadDate('mychecklists')})()" style="width:30px;height:30px;border-radius:8px;border:1px solid #EAE3D8;background:#fff;cursor:pointer;display:grid;place-items:center;color:#A59788">${ic('back','w-4 h-4')}</button>
      <button onclick="S.calWk=0;S.calDate='${today}';S.expandedCl=null;rr();App._lazyLoadDate('mychecklists')" style="padding:0 12px;height:30px;border-radius:8px;font-size:12px;font-weight:700;border:none;cursor:pointer;background:${S.calWk===0&&sel===today?'#13171B':'#F1ECE3'};color:${S.calWk===0&&sel===today?'#fff':'#A59788'}">Today</button>
      <button onclick="(()=>{S.calWk++;const ref2=new Date(todayISO()+'T00:00:00');const dow2=ref2.getDay();ref2.setDate(ref2.getDate()+(dow2===0?-6:1-dow2)+S.calWk*7);const oldDow=new Date(S.calDate+'T00:00:00').getDay()||7;const nd=new Date(ref2);nd.setDate(nd.getDate()+oldDow-1);S.calDate=nd.getFullYear()+'-'+String(nd.getMonth()+1).padStart(2,'0')+'-'+String(nd.getDate()).padStart(2,'0');S.expandedCl=null;rr();App._lazyLoadDate('mychecklists')})()" style="width:30px;height:30px;border-radius:8px;border:1px solid #EAE3D8;background:#fff;cursor:pointer;display:grid;place-items:center;color:#A59788">${ic('chevR','w-4 h-4')}</button>
    </div>
  </div>
  ...
  <div class="cal-strip" style="margin-bottom:12px">
    ${week.map(d=>{
      ...
      return`<button class="csd ${isSel?'act':''}" onclick="S.calDate='${d}';S.expandedCl=null;rr();App._lazyLoadDate('mychecklists')">
        ...
        <div class="csd-dots">
          ${hasDone?`<span style="width:4px;height:4px;border-radius:50%;background:${isSel?'rgba(255,255,255,.7)':'#58996E'}"></span>`:''}
          ${hasLate?`<span style="width:4px;height:4px;border-radius:50%;background:${isSel?'rgba(255,160,160)':'#C25441'}"></span>`:hasPend?`<span style="width:4px;height:4px;border-radius:50%;background:${isSel?'rgba(255,210,80)':'#C9A76B'}"></span>`:''}
        </div>
      </button>`;
    }).join('')}
  </div>
  ...
  ${dayCls.length?`<div style="display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap">
    ${doneN?`<span style="padding:4px 12px;border-radius:20px;background:#E9F1E8;color:#346A47;font-size:12px;font-weight:700">${doneN} submitted</span>`:''}
    ${pendN>0?`<span style="padding:4px 12px;border-radius:20px;background:#F9F1DF;color:#7C5A26;font-size:12px;font-weight:700">${pendN} pending</span>`:''}
    ${lateN>0?`<span style="padding:4px 12px;border-radius:20px;background:#F9E9E3;color:#AC3B2A;font-size:12px;font-weight:700">${lateN} late</span>`:''}
  </div>`:''}
  ...
  <div style="display:flex;flex-direction:column;gap:8px">
    ...
    ${dayCls.length
      ...
      :`<div style="padding:60px 20px;text-align:center;background:#fff;border-radius:18px;border:1px solid #EAE3D8">
          <div style="font-size:40px;margin-bottom:10px">✅</div>
          <div class="fd" style="font-size:17px;font-weight:800;color:#13171B">${sel>today?'Nothing scheduled':'All clear'}</div>
          <p style="font-size:13px;color:#A8998A;margin-top:6px">${sel>today?'No checklists for this date':can('checklists','create')&&DB.checklists.length?'You are not assigned to any checklist. Go to Checklists → edit → assign yourself.':'No checklists scheduled'}</p>
...
function _clFooter(c,date,sub,isPast,isFuture,u,hasEditReq,editApproved){
  ...
  if(sub&&sub.status!=='Editing'&&sub.userId&&sub.userId!==S.uid){
    ...
    return '<span style="font-size:12px;color:#346A47;font-weight:600">✓ Completed by '+esc(by?fullName(by):'a teammate')+(st?' · '+st:'')+'</span><span></span>';
  }
  // Guard: never show Submit if already submitted (catches stale RUN state)
  if(sub&&sub.status!=='Editing'){
    const st=sub.submittedAt?new Date(sub.submittedAt).toLocaleString('en-GB',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}):'';
    const eb=(sub.editCount||0)>0?'<span style="font-size:10px;font-weight:700;background:#F8F0DE;color:#6B4E1F;padding:1px 6px;border-radius:10px">edit #'+sub.editCount+'</span>':'';
    const left='<span style="font-size:12px;color:#A8998A">Submitted '+st+eb+'</span>';
    let right='';
    if(hasEditReq)right='<span style="font-size:12px;font-weight:600;color:#54433C">Edit pending</span>';
    else if(editApproved)right='<button onclick="App._resubmit(\''+cid+'\',\''+date+'\')" class="submit-pill go">Edit &amp; Resubmit</button>';
    else if(sub.status==='Pending Approval'||sub.status==='Pending')right='<span style="font-size:12px;font-weight:600;color:#54433C">Awaiting approval</span>';
    else if(u?.rules?.edit&&u?.managerId)right='<button onclick="App._reqEdit(\''+cid+'\',\''+date+'\')" style="font-size:13px;font-weight:700;color:#346A47;background:#EEE4D5;border:1px solid #8FC1A4;border-radius:8px;cursor:pointer;padding:7px 16px">Request edit</button>';
    return left+right;
  }
  if(!sub){
    if(isPast&&!u?.rules?.past)return '<span style="font-size:12px;color:#7C5A26;font-weight:600">No permission for past dates</span><span></span>';
    if(isFuture&&!u?.rules?.future)return '<span style="font-size:12px;color:#A59788">Scheduled for this date</span><button class="submit-pill no" disabled style="opacity:.4;cursor:not-allowed">Not yet</button>';
    const _hasDraft=(DB.drafts||[]).some(d=>d.checklistId===cid&&d.userId===S.uid&&d.date===date);
    const _draftBtn='<button onclick="App._saveDraft(\''+cid+'\',\''+date+'\')" class="draft-pill" data-cl="'+cid+'" style="padding:8px 18px;border-radius:9px;font-size:13px;font-weight:700;border:1.5px solid #E6DED3;background:#fff;color:#3A312A;cursor:pointer">'+(_hasDraft?'Update draft':'Save as Draft')+'</button>';
    const _leftBlk=_hasDraft?'<span style="display:inline-flex;align-items:center;gap:8px">'+_draftBtn+'<span style="font-size:11px;color:#54433C;font-weight:700">\u2713 Draft saved</span></span>':_draftBtn;
  ...
  if(sub.status==='Editing'){
    return '<span style="font-size:12px;font-weight:700;color:#A5796A">Editing\u2026</span><button onclick="App._submitRun(\''+cid+'\',\''+date+'\')" class="submit-pill go" data-cl="'+cid+'">\u2713 Submit edit</button>';
  }
  return '';
}
// ── Multi-photo helpers ──
...
function _clCard(c,date){
  ...
  const stBar={'On Time':'#58996E','Submitted':'#58996E','Pending':'#C9A76B','Late':'#C25441','Pending Approval':'#54433C','Rejected':'#C25441','Editing':'#A5796A','Upcoming':'#C9AB7C'};

  return`<div class="clc" style="border-top:3px solid ${stBar[st]||'#EAE3D8'}">
    <!-- Header -->
    <button class="clc-hdr" onclick="S.expandedCl=S.expandedCl==='${c.id}'?null:'${c.id}';rr()">
      <div style="flex:1;text-align:left;min-width:0">
        <div class="fd" style="font-size:15px;font-weight:800;color:#13171B">${esc(c.name)}</div>
        <div style="display:flex;align-items:center;gap:8px;margin-top:3px;flex-wrap:wrap">
          ${c.department?`<span style="font-size:12px;color:#A8998A">${esc(c.department)}</span>`:''}
          ${c.anyOne?`<span title="Any one assignee can complete this" style="font-size:10px;font-weight:700;padding:2px 7px;border-radius:10px;background:#F2E9D4;color:#7F6533;flex-shrink:0">👥 Any one</span>`:''}
          ${c.description?`<span style="font-size:11px;color:#A8998A;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:200px">${esc(c.description)}</span>`:''}
          ${(()=>{
            ...
            return`<span style="font-size:11px;font-weight:600;color:${allAnswered?'#54433C':'#A59788'};flex-shrink:0">${answered}/${total} attempted</span>`;
          })()}
        </div>
      </div>
      <div style="display:flex;align-items:center;gap:8px;flex-shrink:0">
        ${(!isSubmitted&&(DB.drafts||[]).some(d=>d.checklistId===c.id&&d.userId===S.uid&&d.date===date))?'<span title="You have a saved draft" style="font-size:10px;font-weight:800;padding:2px 7px;border-radius:10px;background:#F2E9D4;color:#7F6533">📝 Draft</span>':''}
        <span style="font-size:11px;font-weight:700;padding:4px 11px;border-radius:20px;${stCls[st]?'':'background:#F1ECE3;color:#A59788'}" class="${stCls[st]||''}">${st}</span>
        <span style="color:#C9BCAE;transform:rotate(${exp?90:0}deg);transition:transform .2s">${ic('chevR','w-4 h-4')}</span>
      </div>
    </button>
    ...
    ${exp?`<div>
      ${(()=>{
        ...
        return'<div style="padding:12px 16px;border-top:1px solid #F1ECE3;display:flex;flex-direction:column;gap:10px">'+qs.map((q,qi)=>{
          ...
          let inputHtml='';
          if(locked){
            inputHtml='<span style="font-size:13px;font-weight:600;color:#54433C">'+(resp!==null&&resp!==undefined?esc(String(resp)):'<em style="color:#D5C9BC">Not answered</em>')+'</span>';
          } else if(q.type==='answer'){
            inputHtml='<div style="display:flex;flex-wrap:wrap;gap:6px">'+(q.options||[]).map((o,oi)=>`<button onclick="App._setQROpt('${c.id}','${q.id}',${oi})" style="padding:6px 14px;border-radius:20px;border:1.5px solid ${resp===o.text?'#13171B':'#E6DED3'};background:${resp===o.text?'#13171B':'#fff'};color:${resp===o.text?'#fff':'#3A312A'};font-size:12px;font-weight:600;cursor:pointer">${esc(o.text)}</button>`).join('')+'</div>';
          } else if(q.type==='number'){
            inputHtml=`<input type="number" value="${resp??''}" oninput="App._setQR('${c.id}','${q.id}',this.value,true)" onchange="App._setQR('${c.id}','${q.id}',this.value)" placeholder="Enter number…" style="width:120px;padding:6px 12px;border-radius:9px;border:1.5px solid #E6DED3;font-size:13px;outline:none"/>`;
          } else if(q.type==='passfail'){
            inputHtml=`<div style="display:flex;gap:8px"><button onclick="App._setQR('${c.id}','${q.id}','Pass')" style="flex:1;min-height:44px;padding:6px 18px;border-radius:9px;border:1.5px solid ${resp==='Pass'?'#3F7D54':'#E6DED3'};background:${resp==='Pass'?'#E0EDE0':'#fff'};color:${resp==='Pass'?'#3F7D54':'#3A312A'};font-weight:700;font-size:13px;cursor:pointer">Pass</button><button onclick="App._setQR('${c.id}','${q.id}','Fail')" style="flex:1;min-height:44px;padding:6px 18px;border-radius:9px;border:1.5px solid ${resp==='Fail'?'#B3402E':'#E6DED3'};background:${resp==='Fail'?'#F6E2DB':'#fff'};color:${resp==='Fail'?'#B3402E':'#3A312A'};font-weight:700;font-size:13px;cursor:pointer">Fail</button></div>`;
          } else if(q.type==='yesno'){
            inputHtml=`<div style="display:flex;gap:8px"><button onclick="App._setQR('${c.id}','${q.id}','Yes')" style="flex:1;min-height:44px;padding:6px 18px;border-radius:9px;border:1.5px solid ${resp==='Yes'?'#3F7D54':'#E6DED3'};background:${resp==='Yes'?'#E0EDE0':'#fff'};color:${resp==='Yes'?'#3F7D54':'#3A312A'};font-weight:700;font-size:13px;cursor:pointer">Yes</button><button onclick="App._setQR('${c.id}','${q.id}','No')" style="flex:1;min-height:44px;padding:6px 18px;border-radius:9px;border:1.5px solid ${resp==='No'?'#B3402E':'#E6DED3'};background:${resp==='No'?'#F6E2DB':'#fff'};color:${resp==='No'?'#B3402E':'#3A312A'};font-weight:700;font-size:13px;cursor:pointer">No</button></div>`;
          } else if(q.type==='tick'){
            inputHtml=`<div style="display:flex;gap:8px"><button onclick="App._setQR('${c.id}','${q.id}','Done')" style="flex:1;min-height:44px;padding:6px 20px;border-radius:9px;border:1.5px solid ${resp==='Done'?'#3F7D54':'#E6DED3'};background:${resp==='Done'?'#E0EDE0':'#fff'};color:${resp==='Done'?'#3F7D54':'#3A312A'};font-weight:800;font-size:15px;cursor:pointer">✓</button><button onclick="App._setQR('${c.id}','${q.id}','Not done')" style="flex:1;min-height:44px;padding:6px 20px;border-radius:9px;border:1.5px solid ${resp==='Not done'?'#B3402E':'#E6DED3'};background:${resp==='Not done'?'#F6E2DB':'#fff'};color:${resp==='Not done'?'#B3402E':'#3A312A'};font-weight:800;font-size:15px;cursor:pointer">✕</button></div>`;
          }
          ...
          if(!locked){
            ...
            inputHtml+='<div style="margin-top:10px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">'
              +photos.map((ph,pi)=>'<div style="position:relative;display:inline-block"><img src="'+ph+'" alt="Task response photo" onclick="App._bigImg(\''+ph.replace(/'/g,"\\'")+'\')" style="max-width:100px;max-height:72px;border-radius:8px;object-fit:cover;border:1.5px solid #C8E2D0;cursor:pointer"/><button onclick="App._clearQRPhoto(\''+c.id+'\',\''+q.id+'\','+pi+')" style="position:absolute;top:-5px;right:-5px;width:18px;height:18px;border-radius:50%;background:#C25441;border:1.5px solid #fff;color:#fff;font-size:11px;cursor:pointer;display:grid;place-items:center;font-weight:700">×</button></div>').join('')
              +'<label style="display:inline-flex;align-items:center;gap:5px;padding:7px 13px;border-radius:9px;background:'+(photos.length?'#F2F7F1':'#F4F0EA')+';color:'+(photos.length?'#3F7D54':'#3A312A')+';font-size:12px;font-weight:600;cursor:pointer;border:1.5px solid '+(photos.length?'#C8E2D0':'#E6DED3')+'">'
              +ic('cam','w-3.5 h-3.5')+(photos.length?'Add more':'Add photo')+(q.photo&&!photos.length?' <span style="color:#C25441">*</span>':'')
          ...
          return`<div style="background:#FAF7F3;border:1px solid #EDE7DC;border-radius:12px;padding:12px 14px">
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px">
              <span style="font-size:10px;font-weight:700;padding:1px 7px;border-radius:5px;background:${Q_TYPE_BG[q.type]||'#F7F3EE'};color:${Q_TYPE_CLR[q.type]||'#786A5F'}">${TYPE_LABELS[q.type]||q.type}</span>
              <span style="font-size:13px;font-weight:600;color:#13171B">${esc(q.text)}</span>
              ${flags.length?`<span style="margin-left:auto;font-size:11px;color:#A59788">${flags.join(' · ')}</span>`:''}
            </div>
            ${inputHtml}
            ${!locked?`<textarea oninput="App._setQRComment('${c.id}','${q.id}',this.value)" placeholder="${q.comment?'Comment (required)…':'Add a comment (optional)…'}" style="width:100%;box-sizing:border-box;margin-top:8px;padding:8px 10px;border:1.5px solid ${q.comment?'#E4A898':'#E6DED3'};border-radius:9px;font-size:12px;resize:none;outline:none;font-family:inherit;background:#fff" rows="2">${esc(qr.comment||'')}</textarea>`:''}
            ${locked&&qr.comment?`<div style="margin-top:6px;font-size:12px;color:#786A5F;font-style:italic;padding:5px 8px;background:#FAF7F3;border-radius:7px">"${esc(qr.comment)}"</div>`:''}
          </div>`;
        }).join('')+'</div>';
      })()}
      ...
      ${sub?DB.feedback.filter(fb=>fb.checklistId===c.id&&fb.userId===S.uid&&fb.date===date).map(fb=>{
        ...
        return`<div style="background:#EEE5D6;border-top:1px solid #E0CDB9;padding:12px 18px">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
            <span style="font-size:12px;font-weight:700;color:#4A3B34">${ic('msg','w-3.5 h-3.5 inline')} From ${mgr?esc(fullName(mgr)):'Manager'}</span>
            ${!fb.acknowledged?`<button onclick="App._ackFb('${fb.id}')" style="font-size:11px;font-weight:700;padding:3px 10px;border-radius:6px;background:#4A3B34;color:#fff;border:none;cursor:pointer">Acknowledge</button>`:`<span style="font-size:11px;color:#4A3B34">✓ Acknowledged</span>`}
          </div>
          <p style="font-size:13px;color:#42352E;margin:0;line-height:1.5">${esc(fb.text)}</p>
        </div>`;}).join(''):''}
    </div>`:''}
  </div>`;
}
