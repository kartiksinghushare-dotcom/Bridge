...
function allClsPage(){
  ...
  const BC={'Late':'#F43F5E','Pending Approval':'#F97316','On Time':'#A1735F','Submitted':'#A1735F','Pending':'#F59E0B','Rejected':'#9F1239','Editing':'#0EA5E9'};
  ...
  const locHeader=(label,count,muted)=>`<div style="display:flex;align-items:center;gap:7px;margin:18px 0 10px">
      ...
      <span style="font-size:15px;font-weight:800;color:${muted?'#6B7280':'#13171B'}">${esc(label)}</span>
    ...
    <div style="position:sticky;top:52px;z-index:10;background:rgba(247,246,242,.95);backdrop-filter:blur(12px);margin:0 -16px;padding:0 16px 10px">
      <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0">
        ...
        <div style="display:flex;gap:4px;align-items:center">
          ...
          <button onclick="S.filters.aclWk=0;S.filters.aclDate='${today}';S.filters.aclExp=null;rr()" style="padding:5px 10px;border-radius:8px;font-size:11px;font-weight:700;cursor:pointer;border:none;background:${S.filters.aclWk===0&&d===today?'#13171B':'#F6F7F8'};color:${S.filters.aclWk===0&&d===today?'#fff':'#6B7280'}">Today</button>
        ...
        ${week.map(dd=>{
          ...
          return`<button onclick="S.filters.aclDate='${dd}';S.filters.aclExp=null;rr();App._lazyLoadDate('allcl')" class="cal4-d ${isSel?'sel':''}" style="flex:1;min-width:0;display:flex;flex-direction:column;align-items:center;padding:10px 4px 8px;cursor:pointer;border:none;background:${isSel?'#13171B':'transparent'};gap:2px">
            ...
            <div style="display:flex;gap:2px;height:6px">
              ${dots.sub?`<span style="width:5px;height:5px;border-radius:50%;background:${isSel?'rgba(255,255,255,.8)':'#A1735F'}"></span>`:''}
...
function approvalsPage(){
  ...
  return '<div class="fade">'+hdr('Approvals','')
    ...
    +TABS.map(t=>{
      ...
      return '<button onclick="App._setTab(this.dataset.t)" data-t="'+t+'" style="padding:8px 16px;border-radius:10px;font-size:14px;font-weight:600;border:none;cursor:pointer;transition:all .15s;background:'+(active?'#13171B':'transparent')+';color:'+(active?'#fff':TAB_CLR[t]||'#6B7280')+'">'+t+badge+'</button>';
    }).join('')
    +'</div>'
    +'<div class="space-y-3">'
    // ── Approvals tabs ──
            ...
            return '<div class="bg-white rounded-2xl border border-ink-100 shadow-soft p-4">'
              ...
              +(canDecide
                ?'<div class="flex gap-2 mt-3">'
                  +'<button onclick="App._decideApprove(this.dataset.id)" data-id="'+a.id+'" style="flex:1;padding:10px;border-radius:12px;background:#936659;color:#fff;font-weight:600;font-size:14px;border:none;cursor:pointer">Approve</button>'
              ...
              +((canFeedback||isAdmin())
                ?'<div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap">'
                  +(a.type==='Submission'?'<button onclick="App._viewSubFor(this)" data-cl="'+a.checklistId+'" data-uid="'+a.requesterId+'" data-dt="'+a.date+'" style="display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:600;color:#7A5344;background:#FFEAD7;border:1px solid #A7F3D0;border-radius:8px;padding:5px 10px;cursor:pointer">'+ic('eye','w-3.5 h-3.5')+'View</button>':'')
...
App.viewSub=(subId)=>{
  ...
  const qRows=qs.map(q=>{
    ...
    return'<div style="border-radius:10px;border:1px solid '+(hasResp?'#F3E0C8':'#F3F4F6')+';padding:10px 12px;background:'+(hasResp?'#F9FEFB':'#FAFAFA')+'">'
      ...
      +'</div>'
      +(hasResp?'<div style="font-size:13px;font-weight:700;color:#936659">'+esc(String(resp))+'</div>':'<div style="font-size:12px;color:#D1D5DB;font-style:italic">Not answered</div>')
